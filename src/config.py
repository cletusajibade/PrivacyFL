...
ITERATIONS = 10
# LEN_PER_ITERATION: How many datapoints each client gets per iteration (starts at 0). On iteration i, each client
# has (i+1) * LEN_PER_ITERATION samples using equal size datasets for each client in this example
...
"""Security"""
# USE_SECURITY: Implements Diffie-Helman key exchange for added security. Slows runtime slightly, but no effect on
# performance
USE_SECURITY = True
...
# the federated model it receives Subtract your own DP noise from federated model to increase accuracy
SUBTRACT_DP_NOISE = False
# Only subtract DP Noise if adding it to begin with
assert (SUBTRACT_DP_NOISE == False or (SUBTRACT_DP_NOISE == True and USE_DP_PRIVACY == True))
